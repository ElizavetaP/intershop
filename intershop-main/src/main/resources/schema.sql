...
CREATE TABLE IF NOT EXISTS orders (
    ...
    FOREIGN KEY (username) REFERENCES users(username) ON DELETE CASCADE  -- Связь с таблицей users
);
...
CREATE TABLE IF NOT EXISTS cart_item (
    ...
    username VARCHAR(50),                    -- Пользователь (для корзины)
...
-- Индекс для быстрого поиска товаров в корзине пользователя
CREATE INDEX IF NOT EXISTS idx_cart_item_username ON cart_item(username);
-- Индекс для быстрого поиска товаров в заказе
